...
CREATE INDEX idx_nps_surveys_promoters ON nps_surveys(created_at) WHERE score >= 9;
CREATE INDEX idx_nps_surveys_passives ON nps_surveys(created_at) WHERE score >= 7 AND score <= 8;
CREATE INDEX idx_nps_surveys_detractors ON nps_surveys(created_at) WHERE score <= 6;
...
CREATE INDEX idx_support_tickets_open ON support_tickets(status) WHERE status IN ('OPEN', 'IN_PROGRESS', 'WAITING_CUSTOMER', 'REOPENED');
CREATE INDEX idx_support_tickets_type_created ON support_tickets(type, created_at);
...
COMMENT ON COLUMN support_tickets.type IS 'Ticket type categorization for analytics';
