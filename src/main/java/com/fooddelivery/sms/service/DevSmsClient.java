...
public class DevSmsClient implements SmsProvider {
    ...
    // ==================== Template Management ====================
    // DevSMS does not have a template management API.
    // Templates are auto-approved locally without external API calls.

    @Override
    public SmsTemplateSyncResponse registerTemplate(SmsTemplate template) {
        // DevSMS has no template API - auto-approve with local ID
        String localTemplateId = "DEVSMS-" + template.getTemplateCode();
        log.info("Auto-approving DevSMS template locally: code={}, id={}", template.getTemplateCode(), localTemplateId);
        return SmsTemplateSyncResponse.success(localTemplateId, SmsTemplateStatus.APPROVED, getProviderName());
    }

    @Override
    public SmsTemplateSyncResponse updateTemplate(SmsTemplate template) {
        // DevSMS has no template API - auto-approve
        log.info("Auto-approving DevSMS template update: id={}", template.getProviderTemplateId());
        return SmsTemplateSyncResponse.success(template.getProviderTemplateId(), SmsTemplateStatus.APPROVED, getProviderName());
    }

    @Override
    public SmsTemplateSyncResponse deleteTemplate(SmsTemplate template) {
        // DevSMS has no template API - just return success
        log.info("DevSMS template marked as deleted locally: id={}", template.getProviderTemplateId());
    ...
    public SmsTemplateSyncResponse getTemplateStatus(String providerTemplateId) {
        // DevSMS has no template API - always return APPROVED
        return SmsTemplateSyncResponse.success(providerTemplateId, SmsTemplateStatus.APPROVED, getProviderName());
    }
