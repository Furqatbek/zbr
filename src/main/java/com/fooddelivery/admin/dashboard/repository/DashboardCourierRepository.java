...
public interface DashboardCourierRepository extends JpaRepository<Courier, Long> {
     ...
     */
    @Query(value = "SELECT EXTRACT(HOUR FROM business_ts(location_updated_at)), COUNT(DISTINCT id) " +
            "FROM couriers WHERE status IN ('AVAILABLE', 'BUSY') " +
            "AND DATE(business_ts(location_updated_at)) = DATE(:date) " +
            "GROUP BY EXTRACT(HOUR FROM business_ts(location_updated_at)) " +
            "ORDER BY EXTRACT(HOUR FROM business_ts(location_updated_at))",
            nativeQuery = true)
     ...
     */
    @Query(value = "SELECT EXTRACT(DOW FROM business_ts(location_updated_at)) + 1, EXTRACT(HOUR FROM business_ts(location_updated_at)), COUNT(DISTINCT id) " +
            "FROM couriers WHERE status IN ('AVAILABLE', 'BUSY') " +
            "AND location_updated_at BETWEEN :startDate AND :endDate " +
            "GROUP BY EXTRACT(DOW FROM business_ts(location_updated_at)), EXTRACT(HOUR FROM business_ts(location_updated_at))",
            nativeQuery = true)
