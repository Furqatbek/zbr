...
public interface WebSocketMessageLogRepository extends JpaRepository<WebSocketMessageLog, Long> {
    ...
    @Query(value = "SELECT AVG(EXTRACT(EPOCH FROM (delivered_at - published_at)) * 1000) " +
           ...
           "AND is_delivered = true AND delivered_at IS NOT NULL", nativeQuery = true)
    ...
    @Query(value = "SELECT CAST(EXTRACT(EPOCH FROM (delivered_at - published_at)) * 1000 AS BIGINT) " +
           ...
           "ORDER BY (delivered_at - published_at) ASC", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT COUNT(*) * 60.0 / EXTRACT(EPOCH FROM (:end - :start)) " +
           "FROM websocket_message_logs WHERE published_at BETWEEN :start AND :end", nativeQuery = true)
