...
public interface HttpRequestLogRepository extends JpaRepository<HttpRequestLog, Long> {
     ...
     */
    @Query("SELECT COUNT(h) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end")
     ...
     */
    @Query("SELECT AVG(h.responseTimeMs) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end")
    Double getAverageResponseTime(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    /**
     * Get max response time.
     */
    @Query("SELECT MAX(h.responseTimeMs) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end")
    Long getMaxResponseTime(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    /**
     * Get min response time.
     */
    @Query("SELECT MIN(h.responseTimeMs) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end")
     ...
     */
    @Query("SELECT h.responseTimeMs FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end " +
     ...
     */
    @Query("SELECT h.endpoint, COUNT(h) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end " +
     ...
     */
    @Query(value = "SELECT EXTRACT(HOUR FROM business_ts(timestamp)) as hour, COUNT(*), AVG(response_time_ms), " +
           "SUM(CASE WHEN status_code >= 400 THEN 1 ELSE 0 END) " +
           "FROM http_request_logs WHERE timestamp BETWEEN :start AND :end " +
     ...
     */
    @Query("SELECT h.method, COUNT(h) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end " +
           "GROUP BY h.method")
     ...
     */
    @Query("SELECT h.statusCode, COUNT(h) FROM HttpRequestLog h WHERE h.timestamp BETWEEN :start AND :end " +
