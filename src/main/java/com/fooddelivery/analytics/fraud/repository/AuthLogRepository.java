...
public interface AuthLogRepository extends JpaRepository<AuthLog, Long> {
     ...
     */
    @Query("SELECT EXTRACT(HOUR FROM a.createdAt), COUNT(a) FROM AuthLog a " +
            "WHERE a.authType = 'LOGIN' AND a.status = 'FAILED' " +
            "AND a.createdAt BETWEEN :start AND :end " +
    ...
    // ==================== User-Level Security Queries (for specific user) ====================

    @Query("SELECT COUNT(a) FROM AuthLog a WHERE a.userId = :userId AND a.authType = 'LOGIN' " +
            "AND a.createdAt BETWEEN :start AND :end")
    ...
    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM AuthLog a " +
            "WHERE a.userId = :userId AND a.isVpn = true AND a.createdAt BETWEEN :start AND :end")
    Boolean hasVpnUsage(@Param("userId") Long userId,
                        @Param("start") LocalDateTime start,
                        @Param("end") LocalDateTime end);

    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM AuthLog a " +
            "WHERE a.userId = :userId AND a.isTor = true AND a.createdAt BETWEEN :start AND :end")
    Boolean hasTorUsage(@Param("userId") Long userId,
                        @Param("start") LocalDateTime start,
                        @Param("end") LocalDateTime end);

    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM AuthLog a " +
            "WHERE a.userId = :userId AND a.status = 'RATE_LIMITED' AND a.createdAt BETWEEN :start AND :end")
