...
public interface ReferralEventRepository extends JpaRepository<ReferralEvent, Long> {
    ...
    @Query(value = "SELECT u.user_id, " +
            ...
            "FROM (SELECT DISTINCT r.referrer_id as user_id FROM referral_events r WHERE r.created_at BETWEEN :start AND :end " +
            "      UNION SELECT DISTINCT r.referred_user_id FROM referral_events r WHERE r.created_at BETWEEN :start AND :end) u",
            nativeQuery = true)
