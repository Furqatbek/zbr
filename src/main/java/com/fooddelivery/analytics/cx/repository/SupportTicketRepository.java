...
public interface SupportTicketRepository extends JpaRepository<SupportTicket, Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM support_tickets WHERE created_at BETWEEN :start AND :end " +
            "AND resolved_at IS NOT NULL " +
            "AND EXTRACT(EPOCH FROM (resolved_at - created_at)) / 3600 <= :slaHours",
            nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM support_tickets WHERE status NOT IN ('RESOLVED', 'CLOSED') " +
            "AND EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 3600 > :slaHours",
            nativeQuery = true)
     ...
     */
    @Query(value = "SELECT COUNT(*) FROM support_tickets WHERE status NOT IN ('RESOLVED', 'CLOSED') " +
            "AND EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 3600 BETWEEN :warningHours AND :slaHours",
            nativeQuery = true)
     ...
     */
    @Query(value = "SELECT CAST(created_at AS date), " +
            "COUNT(*), " +
