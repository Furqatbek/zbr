    ...
    private void seedCourierRatings() {
        ...
        for (int i = 0; i < 250; i++) {
            courierRatingRepository.save(CourierRating.builder()
                    ...
                    .tipAmount(random.nextDouble() < 0.6 ? BigDecimal.valueOf(random.nextDouble() * 10) : BigDecimal.ZERO)
                    .createdAt(now.minusDays(random.nextInt(30)))
                    .build());
        }
    }
    ...
    private void seedSupportTickets() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            supportTicketRepository.save(SupportTicket.builder()
                    ...
                    .assignedAgentId((long) (random.nextInt(10) + 1))
                    .firstResponseAt(createdAt.plusMinutes(random.nextInt(60)))
                    .resolvedAt(resolvedAt)
                    .closedAt(closedAt)
                    .slaDueAt(createdAt.plusHours(24))
                    .isSlaBreached(slaBreached)
                    .isEscalated(random.nextDouble() < 0.05)
                    .csatScore(status == SupportTicket.TicketStatus.CLOSED ? random.nextInt(5) + 1 : null)
                    .refundIssued(random.nextDouble() < 0.2)
                    .refundAmount(random.nextDouble() < 0.2 ? BigDecimal.valueOf(random.nextDouble() * 50) : null)
                    .createdAt(createdAt)
                    .build());
        }
    }
